   ...
   <div class="container-fluid">
      <div class="row g-0">
         ...
         <main class="col-md-9 ms-sm-auto col-lg-10">
            ...
            <a class="btn btn-primary mb-3" href="{{ url('outlook_list') }}"> Back </a>
            <div class="py-3">
				...
				<div class="card">
					<div class="card-body">
						...
						$attachdata = DB::select("SELECT * FROM `outlook_inbox_attachments` where `mail_id` = ".$maildata->id);
						//echo "<PRE>";print_R($attachdata);die;
						$imageArr = array();
						$messageIdArr = array();
						if(count($attachdata) !== 0){
						    ...
						    if($imageTags->length != 0){
						        ...
						        foreach($imageTags as $k => $tag) {
						        		$srcval = $tag->getAttribute('src');
						        			...
						        			$newsrc = url('public/outlookattachment/'.$imageArr[$k]);
						        			$key = 'outlookattachment/'.$imageArr[$k];
							...
							echo html_entity_decode($maildata->body);
						}
						...
						?>
							<div class="col-md-3">
								...
								if(in_array($ext,$imagextarr)) { ?>
									<a href="{{ url('force_download_outlook/'.$at->id) }}">

										<?php
											$key = 'outlookattachment/'.$at->filename;
							            $client = Storage::disk('s3')->getDriver()->getAdapter()->getClient();
							            $bucket = env('AWS_BUCKET');
								...
								<?php }else { ?>
									<a href="{{ url('force_download_outlook/'.$at->id) }}"><?php echo $at->filename;?></a>
								<?php }
								?>

...
<head>
   ...
   <link rel="stylesheet" href="{{asset('css/views/send-outlook-mail.css')}}">
   <!-- favicon -->
         ...
         <main class="col-md-9 ms-sm-auto col-lg-10">
            ...
            <div class="py-3">
               <div class="hstack border-bottom gap-3 mb-3 pb-3">
                  ...
                  <div class="ms-auto">
                     <a href="{{ route('outlook_list') }}" type="button" class="btn btn-primary shadow-sm fw-500"> Mail List</a> 
            ...
            <div class="py-3">
               <form method="post" onsubmit="return validateData();" action="{{ url('mail_outlook_send') }}" enctype="multipart/form-data">
                  @csrf
                   <div class="row">
            ...
            //   console.log(input.files);
              ...
              if (input.files) {
                  ...
                  if(totalSize >= '10485760'){
                     if(confirm("File Size is more than 10 MB. Are u sure want to upload in One Drive??")){
                        $('.loading').css('display','block');
                        $.ajax({
                           type: 'POST',
                              url: '<?php echo url('onedriveupload');?>',
                              data: form_data,
                              dataType: 'json',
                              //timeout: -1,
                              ...
                              success: function(resp){
                                 ...
                                 $('.loading').css('display','none');
                                  if(resp.status == '1'){
                                    ...
                                    $('#filenamearr').val(resp.filenamearr);
                                    $('#filepatharr').val(resp.filepatharr);
                                    var bodyhtml = '<div dir="ltr">';
                                    for(var f=0;f<resp.weblinkArr.length;f++){
                                       bodyhtml += '<a href="'+resp.weblinkArr[f]+'" style="padding:0px 1px;border-radius:2px;background-color:rgb(243,242,241)" id="m_-948539674449076036OLK_Beautified_432be1ca-5670-0f8d-8ed2-607bb4929a5a" target="_blank">'+resp.filenamearr[f]+'</a><br/>';
                                    }
